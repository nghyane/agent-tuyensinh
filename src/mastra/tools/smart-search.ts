...
export const smartSearchTool = createTool({
  id: 'smartSearch',
  description:
    'Intelligent search with optimized intent detection for Vietnamese and mixed-language queries',
  inputSchema: z.object({
    query: z
      .string()
      .min(1)
      .describe('User query - supports Vietnamese, English, and mixed language'),
  }),
  execute: async ({ context }): Promise<SmartSearchResult> => {
    ...
    try {
      ...
      return {
        ...
        suggestion:
          'Intent detection unavailable. Using general search without specific filtering.',
        qualityAssessment: {
          isReliable: false,
...
function createSuggestion(
  ...
  const isMixedLanguage =
    /[a-zA-Z]/.test(query) &&
    /[àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]/.test(query);
