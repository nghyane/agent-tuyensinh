    ...
    def _convert_to_intent_rules(self) -> List[IntentRule]:
        ...
        for rule_data in self.rules_data["rules"]:
            try:
                ...
                print(
                    f"  ✅ {intent_rule.intent_id}: "
                    f"{len(intent_rule.keywords)} keywords, "
                    f"{len(intent_rule.patterns)} patterns"
                )

            except Exception as e:
                print(
                    f"  ❌ Failed to convert rule "
                    f"{rule_data.get('intent_id', 'unknown')}: {e}"
                )
                continue
            ...
            metadata.update(
                {
                    ...
                    "total_keywords": sum(
                        len(rule.keywords) for rule in self.loaded_rules
                    ),
                    "total_patterns": sum(
                        len(rule.patterns) for rule in self.loaded_rules
                    ),
                }
            )
...
def get_default_demo_rules() -> List[IntentRule]:
    ...
    return [
        ...
        IntentRule(
            ...
            patterns=[
                r"(?:campus|cơ sở).*(?:ở đâu|where)",
                r"thư viện.*(?:giờ|hours)",
            ],
            weight=1.2,
        ...
        IntentRule(
            ...
            patterns=[
                r"điểm chuẩn.*(?:bao nhiêu|2024|2025)",
                r"(?:yêu cầu|requirements).*(?:đầu vào|admission)",
            ],
            weight=1.3,
