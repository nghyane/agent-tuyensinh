...
class FPTUniversityKnowledgeManager:
    ...
    def load_knowledge_base(self, recreate: bool = False) -> None:
        ...
        for attempt in range(max_retries):
            try:
                print(f"📚 Loading knowledge base (attempt {attempt + 1}/{max_retries})...")
    ...
    def search_knowledge(self, query: str, num_documents: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        return [{"content": doc.content, "metadata": getattr(doc, 'metadata', {})} for doc in results]
