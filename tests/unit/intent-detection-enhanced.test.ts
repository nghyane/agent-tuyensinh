...
async function runEnhancedIntentDetectionTest() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (testCase.minConfidence && confidence < testCase.minConfidence) {
        passed = false;
        reasons.push(`Low confidence: ${(confidence * 100).toFixed(1)}% < ${(testCase.minConfidence * 100).toFixed(1)}%`);
      }
      ...
      if (passed) {
        console.log(`   ✅ PASS: ${actualIntent} (${(confidence * 100).toFixed(1)}%) - ${executionTime}ms`);
      } else {
  ...
  console.log(`   Avg response time: ${(results.reduce((sum, r) => sum + r.executionTime, 0) / results.length).toFixed(1)}ms`);
  ...
  for (const category of categories) {
    ...
    console.log(`   ${emoji} ${category}: ${categoryPassed}/${categoryResults.length} (${categoryAccuracy.toFixed(1)}%)`);
  }
  ...
  if (avgConfidence < 0.8) {
    console.log(`   ⚠️  Low confidence (${(avgConfidence * 100).toFixed(1)}%) - enhance model training`);
  }
  ...
  if (failedTests.length > 0) {
    console.log(`   🔍 Review failed tests: ${failedTests.map(r => r.testCase.description).join(', ')}`);
  }
