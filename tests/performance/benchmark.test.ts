...
async function runPerformanceBenchmarks() {
  ...
  for (const query of queries) {
    const { hybridIntentDetectionService } = await import("../../src/services/intent-detection-hybrid.js");
    ...
    console.log(`✅ "${query}"`);
    console.log(`   Intent: ${intent.id} | Confidence: ${(intent.confidence * 100).toFixed(1)}% | Method: ${intent.method} | Time: ${responseTime.toFixed(0)}ms`);
  }
  ...
  for (const query of queries) {
    ...
    console.log(`${status} "${query.substring(0, 35)}..." | ${responseTime.toFixed(2)}ms | ${(confidence * 100).toFixed(1)}%`);
  }
  ...
  if (agentResults.length > 0) {
    const agentSuccessRate = (agentResults.filter(r => r.success).length / agentResults.length) * 100;
  ...
  if (regexResults.length > 0) {
    const regexSuccessRate = (regexResults.filter(r => r.success).length / regexResults.length) * 100;
  ...
  if (suites.length >= 2) {
    ...
    if (agentSuite && regexSuite) {
      ...
      console.log(`   💡 Trade-off: ${speedRatio.toFixed(1)}x time cost for ${accuracyGain.toFixed(1)}% accuracy gain`);
    }
  }
